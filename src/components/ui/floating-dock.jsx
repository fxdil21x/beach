...
export function FloatingDock({
  ...
  return (
    <div className={cn('floating-dock-wrapper w-full max-w-lg mx-auto flex justify-center pointer-events-none', className)}>
...
function FloatingDockCore({ items, className }) {
  ...
  return (
    ...
    >
      {items.map((item) => (
        <DockIcon key={item.to || item.title} mouseX={mouseX} item={item} />
      ))}
    </motion.div>
  );
}
        ...
        >
          ...
          {isActive && (
            <motion.div
              ...
              className="absolute inset-0 rounded-full bg-blue-50/95 dark:bg-blue-950/70 shadow-sm ring-1.5 ring-blue-500/30"
            />
          )}
