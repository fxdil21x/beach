...
export default function CommonModal({
  ...
  const modal = (
    <div
      className={`${positionClass} z-[9999] flex items-center justify-center p-4 touch-none overscroll-contain pointer-events-auto animate-in fade-in duration-300`}
      style={{
    ...
    >
      <div
        className={`relative flex max-h-[calc(100%-2rem)] w-full ${maxWidth} flex-col overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 text-slate-900 overscroll-contain pointer-events-auto animate-in zoom-in-95 slide-in-from-bottom-3 duration-300 ease-out`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Header ── */}
        {(title || IconComponent) && (
          <div className="flex items-start justify-between border-b border-slate-100 bg-white px-5 py-4 shrink-0 pointer-events-auto">
        ...
        {/* ── Scrollable Body ── */}
        <div className="flex-1 overflow-y-auto overscroll-contain px-5 py-4 space-y-3.5 pointer-events-auto">
          {children}
        </div>

        {/* ── Footer ── */}
        <div className="border-t border-slate-100 bg-white px-4 py-3.5 shrink-0 pointer-events-auto">
          {footerActions}
        </div>
      </div>
    </div>
